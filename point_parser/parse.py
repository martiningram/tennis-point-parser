...
class FormatFunctions:
    ...
    match_over_fun: Callable[[MatchState], bool]
    service_game_over: Callable[[int, int], bool]

    # Takes total games & first server and returner
    cur_server: Callable[[int, str, str], Tuple[str, str]]

    is_final_set: Callable[[int], bool]
    is_tiebreak_fun: Callable[[int, int], bool]
    tiebreak_over: Callable[[int, int], bool]
    tiebreak_server: Callable[[MatchState], Tuple[str, str]]
...
def player_wins_set(
    ...
    if not format_functions.match_over_fun(match_state):
...
def player_wins_game(match_state: MatchState,
    ...
    match_state.cur_game_score[winning_player] += 1
    match_state.reset_game_score()
    match_state.total_games_played += 1

    # Set server and returner
    match_state.server, match_state.returner = format_functions.cur_server(
    ...
    if format_functions.set_win_condition(games_winner, games_loser):
...
def advance_tiebreak(server_won: bool,
    ...
    match_state.server, match_state.returner = \
        format_functions.tiebreak_server(match_state)
...
def get_point_winner_loser(server, returner, server_won):
    ...
    winning_player, losing_player = (
        (match_state.server, match_state.returner) if server_won
        else (match_state.returner, match_state.server))
